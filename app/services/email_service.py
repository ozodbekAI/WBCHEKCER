  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;padding:40px 0">
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);max-width:520px;width:100%">
        ...
        <!-- Header -->
        <tr>
          <td style="background:linear-gradient(135deg,#0f172a,#1f2937);padding:36px 40px 32px;text-align:center">
          ...
          <td style="padding:36px 40px">
            <p style="margin:0 0 20px;font-size:16px;color:#374151;line-height:1.6">
              <strong>{inviter_name}</strong> приглашает вас присоединиться к магазину
	              ...
	              в роли <strong style="color:#111827">{role_label}</strong>.
            </p>
            ...
            <table cellpadding="0" cellspacing="0" width="100%">
              <tr>
                <td align="center" style="padding-bottom:28px">
                  <a href="{invite_link}"
                     style="display:inline-block;background:linear-gradient(135deg,#111827,#1f2937);
...
def _build_registration_password_html(password: str, company_name: str = "AVEMOD") -> str:
    return f"""<!DOCTYPE html>
<html lang="ru">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;padding:40px 0">
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);max-width:520px;width:100%">
        <tr>
          <td style="background:linear-gradient(135deg,#0f172a,#1f2937);padding:36px 40px 32px;text-align:center">
            <h1 style="margin:0;font-size:24px;font-weight:800;color:#ffffff;line-height:1.25">{company_name}</h1>
            <p style="margin:10px 0 0;font-size:14px;color:#cbd5e1">Доступ к аккаунту</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px">
            <p style="margin:0 0 20px;font-size:15px;color:#374151;line-height:1.6">
              Ваш временный пароль для входа в систему:
            </p>
            <div style="margin:0 0 20px;padding:16px;border-radius:12px;border:1px solid #cbd5e1;background:#f8fafc;text-align:center">
              <span style="font-size:24px;font-weight:800;letter-spacing:1px;color:#111827">{password}</span>
            </div>
            <p style="margin:0;font-size:13px;color:#6b7280;line-height:1.6">
              После входа сразу смените пароль в разделе Profile. Если запрос делали не вы — игнорируйте это письмо.
            </p>
          </td>
        </tr>
      </table>
...
def _build_email_verification_html(code: str, company_name: str = "AVEMOD") -> str:
    return f"""<!DOCTYPE html>
<html lang="ru">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);max-width:520px;width:100%">
        <tr>
          <td style="background:linear-gradient(135deg,#0f172a,#1f2937);padding:36px 40px 32px;text-align:center">
            <h1 style="margin:0;font-size:24px;font-weight:800;color:#ffffff;line-height:1.25">{company_name}</h1>
            <p style="margin:10px 0 0;font-size:14px;color:#cbd5e1">Подтверждение аккаунта</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px">
            <p style="margin:0 0 20px;font-size:15px;color:#374151;line-height:1.6">
              Введите этот 6-значный код для активации аккаунта:
            </p>
            <div style="margin:0 0 20px;padding:16px;border-radius:12px;border:1px solid #cbd5e1;background:#f8fafc;text-align:center">
              <span style="font-size:32px;font-weight:800;letter-spacing:6px;color:#111827">{code}</span>
            </div>
            <p style="margin:0;font-size:13px;color:#6b7280;line-height:1.6">
...
def send_registration_password_email(
    ...
    """Send registration temporary password email."""
    from email.utils import formataddr

    brand = company_name or "AVEMOD"
    subject = f"{brand}: временный пароль для входа"
    from_display = formataddr((brand, settings.SMTP_FROM))
    ...
    msg["From"] = from_display
    msg["To"] = to_email
    msg.attach(MIMEText(_build_registration_password_html(password, company_name), "html", "utf-8"))
        ...
        if settings.SMTP_TLS:
            ...
            with smtplib.SMTP_SSL(settings.SMTP_HOST, settings.SMTP_PORT) as server:
                if settings.SMTP_USER:
                    server.login(settings.SMTP_USER, settings.SMTP_PASSWORD)
                server.sendmail(settings.SMTP_FROM, to_email, msg.as_string())
        logger.info("Registration password email sent to %s", to_email)
    except Exception as exc:
...
def send_email_verification_code(
    ...
    """Send 6-digit verification code for account activation."""
    from email.utils import formataddr

    brand = company_name or "AVEMOD"
    subject = f"{brand}: код подтверждения аккаунта"
    from_display = formataddr((brand, settings.SMTP_FROM))

    if not settings.SMTP_HOST:
        logger.warning(
            "\n" + "=" * 60 +
            f"\n📧  VERIFY EMAIL (SMTP not configured)"
            f"\n   To:   {to_email}"
            f"\n   Code: {code}"
            "\n" + "=" * 60
        )
    ...
    msg["From"] = from_display
    msg["To"] = to_email
    msg.attach(MIMEText(_build_email_verification_html(code, company_name), "html", "utf-8"))
    ...
    try:
        ...
        logger.error("Failed to send verification code to %s: %s", to_email, exc)
        raise
